...
#
# Usage:
#   bash setup_global.sh ~/.vibe        # ein Agent-Dir
#   bash setup_global.sh ~/.claude ~/.codex   # mehrere
#   bash setup_global.sh --all          # alle bekannten Agent-Dirs unter $HOME
#   bash setup_global.sh                 # ohne Arg = --all
#
# (Alter Name setup_global_tickets.sh existiert weiter als Alias-Wrapper.)
#
...
HDR
  ...
  if _fetch "hooks/ticket-mover.sh" "$AGENT_DIR/hooks/ticket-mover.sh"; then
